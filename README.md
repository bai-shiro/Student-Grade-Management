...
1. 保持终端在项目根目录(StudentGradeManagementBackend)，执行启动命令：
   ```bash
   dotnet run
   ```
...
3. 核心操作演示：
   - **学生管理**：点击“学生管理”→“添加学生”，录入学号、姓名、所属班级等信息，提交后可在列表查看；
...
1. 管理员先创建学生账号：
   - 进入“用户管理”→“新增用户”，选择角色“学生”，输入用户名（如学生学号“2023065732”），密码（如“123456”），关联对应学生信息；
2. 退出管理员登录，重新在登录页输入学生账号：`2023065732`，密码：`123456`，点击“登录”；
3. 登陆后即可查看本人已录入成绩，若无成绩，则显示暂时没有成绩信息。

### 3. 演示注意事项

- 演示过程中若出现“数据库连接失败”：检查MySQL服务是否启动（Windows：服务列表中查看“MySQL80”状态；），或连接字符串中的账号/密码/端口是否正确；
- DataBase的create_table.sql测试数据出现问题可根据table定义重新编写
...
1. 本项目为极为简易的课程实践项目，仅供课程学习交流，有许多待改进的地方，如需加密存储密码（如MD5/SHA256），部分统计信息功能不完备等等未实现；
...
7. DataBase的create_table.sql测试数据建议重新编写
8. 最后还是决定不上传报告了
...
![1774272066777](image/README/1774272066777.png)

![1774272172401](image/README/1774272172401.png)
